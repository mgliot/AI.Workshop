...
internal class RagWorkflowExamples : IDisposable
{
    ...
    {
        // Load settings from appsettings.json (with fallback to defaults)
        ...
        _chatClient = new OllamaApiClient(aiSettings.GetOllamaUri(), aiSettings.ChatModel);
        _client = _chatClient;

        // OllamaApiClient implements IEmbeddingGenerator - create a separate client for embeddings
        _embeddingClient = new OllamaApiClient(aiSettings.GetOllamaUri(), aiSettings.EmbeddingModel);
